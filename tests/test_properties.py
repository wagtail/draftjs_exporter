...
from tests.strategies import content_states, dangerous_content_states, markdown_text
...
class TestImporterCrashSafety(unittest.TestCase):
    """`markdown_to_content_state` must not raise on adversarial input.

    The importer is the reverse direction of the export pipeline: it walks
    untrusted Markdown text character-by-character to produce ranges.
    Unlike ContentState (which Draft.js editors produce in a constrained
    shape), Markdown is often hand-written or generated by other tools,
    so the importer must degrade gracefully on adversarial inputs -
    unterminated links, mismatched style markers, malformed HTML tags,
    deeply nested syntax - rather than crash or produce malformed output.

    This mirrors `TestRenderCrashSafety` for the export direction.
    """

    @given(markdown=markdown_text)
    @settings(deadline=None)
    # Pinned examples of known-tricky inputs so they always run even if
    # Hypothesis's random search would not stumble on them again.
    @example(markdown="")
    @example(markdown="\n")
    @example(markdown="[unclosed")
    @example(markdown="*unclosed")
    @example(markdown="**unclosed")
    @example(markdown="<unclosed>")
    @example(markdown="```\nunclosed code fence")
    @example(markdown="[text](unclosed")
    @example(markdown="<u>unclosed tag")
    @example(markdown="\\")  # Trailing backslash at end of input.
    @example(markdown="![alt](unclosed")
    @example(markdown="> unclosed blockquote")
    def test_importer_never_raises(self, markdown):
        result = markdown_to_content_state(markdown)

        # Result is always well-formed ContentState, regardless of input.
        self.assertIsInstance(result, dict)
        self.assertIsInstance(result.get("blocks"), list)
        self.assertIsInstance(result.get("entityMap"), dict)

        for block in result["blocks"]:
            self.assertIsInstance(block.get("key"), str)
            self.assertIsInstance(block.get("text"), str)
            self.assertIsInstance(block.get("type"), str)
            self.assertIsInstance(block.get("depth"), int)
            self.assertIsInstance(block.get("inlineStyleRanges"), list)
            self.assertIsInstance(block.get("entityRanges"), list)

            # Style ranges have valid offset/length/style fields.
            for style_range in block["inlineStyleRanges"]:
                self.assertIsInstance(style_range.get("offset"), int)
                self.assertIsInstance(style_range.get("length"), int)
                self.assertIsInstance(style_range.get("style"), str)

            # Entity ranges have valid offset/length/key fields.
            for entity_range in block["entityRanges"]:
                self.assertIsInstance(entity_range.get("offset"), int)
                self.assertIsInstance(entity_range.get("length"), int)
                self.assertIsInstance(entity_range.get("key"), int)

            # Every entity range's key resolves in the entity map.
            for entity_range in block["entityRanges"]:
                self.assertIn(str(entity_range["key"]), result["entityMap"])
