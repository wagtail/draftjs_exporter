...
class TestEscapeTextAnywhere(unittest.TestCase):
    def test_backslash_escaped_first(self):
        self.assertEqual(md_escape_text("a\\*b"), "a\\\\\\*b")

    def test_emphasis_chars(self):
        self.assertEqual(md_escape_text("*em* _em_"), "\\*em\\* \\_em\\_")

    def test_intraword_underscores_not_escaped(self):
        self.assertEqual(md_escape_text("draftjs_exporter"), "draftjs_exporter")
        self.assertEqual(md_escape_text("a__b"), "a__b")

    def test_boundary_underscores_escaped(self):
        self.assertEqual(md_escape_text("_foo_"), "\\_foo\\_")
        self.assertEqual(md_escape_text("a_ b"), "a\\_ b")
        # Conservative: inert between spaces, but escaped harmlessly.
        self.assertEqual(md_escape_text("a _ b"), "a \\_ b")

    def test_code_backtick(self):
        self.assertEqual(md_escape_text("a`b"), "a\\`b")

    def test_brackets(self):
        self.assertEqual(md_escape_text("[x](y)"), "\\[x\\](y)")

    def test_angle_bracket(self):
        self.assertEqual(md_escape_text("<script>"), "\\<script>")

    def test_ampersand_not_escaped(self):
        # Entity references decode downstream but cannot inject markup.
        self.assertEqual(md_escape_text("&copy;"), "&copy;")

    def test_parentheses_not_escaped(self):
        self.assertEqual(md_escape_text("(hi)"), "(hi)")
...
class TestEscapeTextLineStart(unittest.TestCase):
    def test_hash_at_line_start(self):
        self.assertEqual(md_escape_text("# hi", at_line_start=True), "\\# hi")

    def test_hash_not_at_line_start(self):
        self.assertEqual(md_escape_text("# hi"), "# hi")

    def test_dash_plus(self):
        self.assertEqual(md_escape_text("- a", True), "\\- a")
        self.assertEqual(md_escape_text("+ a", True), "\\+ a")
    ...
    def test_ordered_list_marker(self):
        self.assertEqual(md_escape_text("1. item", True), "1\\. item")
        self.assertEqual(md_escape_text("12) item", True), "12\\) item")

    def test_ordered_list_marker_ten_digits_not_escaped(self):
        self.assertEqual(md_escape_text("1234567890. x", True), "1234567890. x")
    ...
    def test_embedded_newline(self):
        self.assertEqual(md_escape_text("a\n- b"), "a\n\\- b")
        self.assertEqual(md_escape_text("a\n# b"), "a\n\\# b")

    def test_empty_string(self):
        self.assertEqual(md_escape_text("", at_line_start=True), "")
...
class TestEscapeTextLineEndings(unittest.TestCase):
    ...
    def test_lone_cr_is_line_start(self):
        self.assertEqual(md_escape_text("a\r# b"), "a\r\\# b")
        self.assertEqual(md_escape_text("a\r- b"), "a\r\\- b")

    def test_mixed_line_endings_preserved(self):
        self.assertEqual(md_escape_text("a\rb\r\nc\nd"), "a\rb\r\nc\nd")
        self.assertEqual(md_escape_text("a\n\r\n# b"), "a\n\r\n\\# b")
...
class TestEscapeTextLeadingSpaces(unittest.TestCase):
    def test_hash_after_spaces(self):
        self.assertEqual(md_escape_text("   # heading", True), "   \\# heading")

    def test_unordered_list_marker_after_spaces(self):
        self.assertEqual(md_escape_text("  - item", True), "  \\- item")

    def test_blockquote_marker_after_spaces(self):
        self.assertEqual(md_escape_text(" > quote", True), " \\> quote")
    ...
    def test_code_fence_after_spaces(self):
        self.assertEqual(md_escape_text("   ~~~", True), "   \\~~~")
    ...
    def test_four_or_more_spaces_still_escaped(self):
        ...
        self.assertEqual(md_escape_text("     # x", True), "     \\# x")
    ...
    def test_spaces_not_at_line_start_unchanged(self):
        self.assertEqual(md_escape_text("  # x"), "  # x")
...
class TestEscapeLinkDestination(unittest.TestCase):
    def test_plain_url_unchanged(self):
        self.assertEqual(
            md_escape_link_destination("https://example.com/a?b=1&c=2"),
            "https://example.com/a?b=1&c=2",
        )

    def test_parentheses_escaped(self):
        self.assertEqual(
            md_escape_link_destination("https://example.com/a_(b)"),
            "https://example.com/a_\\(b\\)",
        )

    def test_backslash_escaped(self):
        self.assertEqual(md_escape_link_destination("a\\b"), "a\\\\b")

    def test_space_percent_encoded(self):
        self.assertEqual(md_escape_link_destination("a b"), "a%20b")

    def test_control_char_percent_encoded(self):
        self.assertEqual(md_escape_link_destination("a\nb"), "a%0Ab")
