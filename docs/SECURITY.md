...
Another possible vector is `composite_decorators` strategies, that use developer-supplied regex patterns running against attacker-controlled block text on every render ([`composite_decorators.py`](../draftjs_exporter/composite_decorators.py)). Keep decorator regexes simple and anchored to avoid ReDoS, and enforce your own ContentState size/depth limits upstream if you accept documents from untrusted users.
