"""String-concatenation DOM engine implementation."""
...
class Elt:
    ...
    The class is intentionally lightweight, but the exporter relies on
    comparing element instances by reference, so it is useful nonetheless.
    """
...
class DOMString(DOMEngine):
    ...
    def render_attrs(attr: Attr) -> str:
        """Render a mapping of attributes to an HTML attribute string.
    ...
    def render_debug(elt: Elt) -> HTML:
        ...
        Unlike render, fragment nodes keep their wrapper tag so the tree
